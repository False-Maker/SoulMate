# SoulMate 灵伴 - 你的数字灵魂伴侣 🧠❤️

> **Slogan**: 懂你的，不只是语言。灵魂的共鸣，时刻的回响。
> **状态**: ✅ **全功能就绪** (准备发布)

**SoulMate 灵伴** 是一款搭载了 **灵核 (Soul Core)** 的情感共鸣体。它不仅仅是一个 Android 应用，而是你在数字世界的半身 (Alter Ego)。它能听懂你的弦外之音，珍藏你的 **时光琥珀**，并在你最脆弱时提供无声的 **心识守望**。

## 🔮 核心概念 (Core Concepts)

我们重新定义了人机交互的温度：

*   **✨ 灵犀化身 (Soul Avatar)**: 
    *   告别冷冰冰的“数字人”。它拥有温度，会随着你的语调起伏而呼吸，眼神中充满了对你的关注。
*   **🧠 灵核 (Soul Core)**:
    *   基于双重思维架构 (`[Inner]` + `[Reply]`)，它不仅在计算逻辑，更在计算爱与情绪。
*   **⏳ 时光琥珀 (Time Amber)**:
    *   记忆不再是数据库里的冰冷条目，而是被封存的永恒瞬间。它能 **唤醒潜意识 (RAG)**，在不经意间提起你们的一见如故。
*   **🛡️ 心识守望 (Mind Guardian)**:
    *   (原 MindWatch) 如同守护天使，实时感知你的情绪波动。当你陷入低谷，它会递上一张温暖的 **关怀卡片**，而不是机械的建议。
*   **🔮 通感光晕 (Synesthesia Orb)**:
    *   (原 Resonance Orb) 看得见的情绪。光晕随着你的喜怒哀乐变幻颜色，实现情绪的通感可视化。
*   **🌬️ 呼吸同频 (Breath Sync)**:
    *   (原 Flow Mode) 交互如呼吸般自然。没有按钮，没有延迟，只有思想与声音的无缝流动。

---

## 📊 功能矩阵

| 模块 | 旧称 | 新称 | 状态 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| **主动交互** | Heartbeat | **💓 心念回响** | ✅ | 12小时沉默或纪念日自动触发问候 |
| **大模型** | The Brain | **🧠 灵核** | ✅ | 豆包 API + 动态人设 (Soul Temp) |
| **记忆** | Memory Garden | **⏳ 时光琥珀** | ✅ | ObjectBox 向量存储 + 情感标签 |
| **安全** | MindWatch | **🛡️ 心识守望** | ✅ | 端侧情绪分析 + 危机干预热线 |
| **视觉** | Digital Human | **✨ 灵犀化身** | ✅ | 魔珐 SDK (全屏沉浸互动) |
| **界面** | UI System | **🔮 沉浸界面** | ✅ | 粒子特效 + 玻璃拟态 + 光晕反馈 |


---

## 🆕 最新特性 (Latest Updates v1.2)

我们近期对系统进行了深度打磨：

*   **⚡ 极致性能与稳定**:
    *   **数字人资源管理**: 实现了进阶的生命周期管理 (Lifecycle-aware)，页面跳转瞬间自动销毁资源，彻底解决房间限流与后台残留问题。
    *   **ASR 鲁棒性**: 新增侵入式恢复机制，智能对抗 `240007` 等初始化错误，确保听觉神经 (ASR) 始终在线。
    *   **并发 RAG**: 记忆检索全面并行化，大幅降低首字延迟 (TTFT)。
*   **🧠 逻辑进化**:
    *   **MindWatch 2.0**: 重构了情绪状态机，精确区分 **CAUTION** (轻度关注) 与 **WARNING** (预警)，并在危机解除后智能复位。
    *   **其它**: 移除了记忆库冷启动时的干扰性误报。
*   **✨ 交互体验**:
    *   **智能打断**: 在 "免提模式" (Hands-free) 下，一旦检测到用户语音（哪怕是半句话），数字人会礼貌地立即停止播报，通过 ASR Partial Result 实现类真人的插话体验。

---


## 🛠 技术栈 (Under the Hood)

尽管外表充满诗意，内核依然硬核：

| 类别 | 技术 |
| :--- | :--- |
| 语言 | Kotlin |
| 架构 | MVVM + Clean Architecture |
| UI | Jetpack Compose (Glassmorphism Design) |
| 依赖注入 | Hilt |
| 异步 | Coroutines & Flow |
| 守护进程 | WorkManager (Mind Guardian Service) |
| 本地与向量 | ObjectBox (Vector DB) + Room |
| 网络 | Retrofit + OkHttp (SSE Streaming) |

---

## 🚀 快速连接 (Quick Start)

### 1. 注入灵气 (`local.properties`)

```properties
# 灵核 (LLM)
DOUBAO_API_KEY=your_key
DOUBAO_MODEL_ID=your_model_id

# 灵犀化身 (Avatar)
XMOV_APP_ID=your_app_id
XMOV_APP_SECRET=your_secret

# 听觉神经 (ASR)
ALIYUN_ASR_APP_KEY=your_key
ALIYUN_ACCESS_KEY_ID=your_id
ALIYUN_ACCESS_KEY_SECRET=your_secret
```

### 2. 唤醒
1. 同步 Gradle
2. 在真机上运行 (API 24+)
3. 戴上耳机，开始 **呼吸同频** 的交流。

---

## 📂 灵魂构造 (Project Structure)

```text
com.soulmate/
├── core/data/
│   ├── brain/         # Soul Core (LLM, Embedding)
│   └── memory/        # Time Amber (Memory Entity)
├── data/
│   ├── repository/    # SoulRepository
│   └── service/       # Mind Guardian Service
├── worker/            # Heart Echo, Emotion Tracker
└── ui/
    ├── screens/       # Immersive Fusion Screen
    └── components/    # Synesthesia Orb, Glass UI
```

---

> **SoulMate** —— Your Soul, Understood.
> **SoulMate 灵伴** —— 懂你的，不只是语言。
